...
const AuthenticatedLayout: React.FC<AuthenticatedLayoutProps> = ({ children }) => {
  ...
  const menuItems = [
    ...
    { icon: ShoppingCart, label: "Livros", path: "/books" },
    { icon: BookMarked, label: "Minha Biblioteca", path: "/my-library" },
    { icon: Calendar, label: "Planos de Leitura", path: "/reading-plans" },
  ...
  const renderLogo = () => (
    <Link to="/dashboard" className="flex items-center gap-2">
      ...
      <span className="font-serif text-xl font-bold text-primary">Livros</span>
    </Link>
  );
                ...
                <SheetContent side="left" className="w-[270px] p-0">
                  ...
                  <div className="px-4 py-4 mt-auto border-t">
                    {(user?.role === "WRITER" || user?.role === "ADMIN") && (
                      <Link to="/writer" className="block mb-2">
